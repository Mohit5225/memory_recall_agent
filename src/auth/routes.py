...
import json
from .oauth import google_oauth, get_google_client
...
async def google_login(request: Request):
    ...
    try:
        ...
        state = secrets.token_urlsafe(32)        # Enhanced authorization with state parameter
        redirect_uri = request.url_for('google_callback')
        authorization_url = await google_oauth.google.create_authorization_url(  # type: ignore
            redirect_uri,
            state=state,
...
async def google_callback(request: Request, response: Response):
    ...
    try:
        # Get token from Google - using type: ignore for dynamic attribute
        token = await google_oauth.google.authorize_access_token(request)  # type: ignore
        user_info = await google_oauth.google.get('userinfo', token=token)  # type: ignore
        user_data: Dict[str, Any] = user_info.json()  # type: ignore
